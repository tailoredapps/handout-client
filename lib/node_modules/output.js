"use strict"

import chalk from 'chalk'
import stripAnsi from 'strip-ansi'

export const LEVEL_OK = 'ok'
export const LEVEL_INFO = 'info'
export const LEVEL_ERROR = 'error'
export const LEVEL_PIPE = 'pipe'

const colors = new Map([
    [LEVEL_ERROR, chalk.bold.red],
    [LEVEL_INFO, chalk.bold.grey],
    [LEVEL_OK, chalk.bold.green]
])

function pad(str, max = 15, char = ' ') {
    const len = stripAnsi(str).length

    return len < max ? str + (new Array(max - len)).join(char) : str
}

export default function output(data, fromRemote = false) {
    let level, message

    if (typeof data === 'string') {
        level = LEVEL_INFO
        message = data
    }
    else {
        level = data.error ? LEVEL_ERROR : data.level
        message = data.message
    }

    let str = fromRemote && level !== LEVEL_PIPE ? '> ' : '  '

    str += level === LEVEL_PIPE ? '    ' : `[${colors.get(level)(level)}]`

    console.log(`${pad(str)} ${message}`)
}

export function outputOk(message, ...args) {
    output({
        level: LEVEL_OK,
        message
    }, ...args)
}
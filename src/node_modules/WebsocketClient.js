...
import fs from 'fs'
import { MSG_LEVEL_ERROR, MSG_LEVEL_OK, MSG_LEVEL_PROCESS } from 'handout-constants'
import ursa from 'ursa'
import Websocket from 'ws'

import output, { outputStyles, processResponse } from 'output'
...
class WebsocketClient {
    ...
    return new Promise((resolve, reject) => {
      ...
      this.socket.on('message', (data) => {
        try {
          ...
          if (level === MSG_LEVEL_PROCESS) {
            processResponse(message, this._command)
          } else {
            output(message, level, true)
          }
        } catch (e) {
          output(`Failed to parse server response. Something weird might be happening. Error message: ${e.message}`, MSG_LEVEL_ERROR)
      ...
      this.socket.on('close', (code, message) => {
        if (code > 1000) {
          output(`Command failed with code ${outputStyles.error(code)}.`, MSG_LEVEL_ERROR)
        } else {
          output(`Command completed, closing connection to ${outputStyles.url(url)}.`, MSG_LEVEL_OK)
        }
  ...
  sendCommand ({ command, keyfile, payload, debug }) {
    ...
    this.socket.send(JSON.stringify({ username, clientVersion, debug, cryptData }))
  }

...
import {
  PERMISSION_GRANT,
  PERMISSION_DENY,
  PERMISSION_CATEGORY_DEPLOY,
  PERMISSION_CATEGORY_PERMISSION_MANAGEMENT,
  PERMISSION_CATEGORY_USER_LIST,
  PERMISSION_CATEGORY_USER_MANAGEMENT
...
export default function getCommands (defaults) {
  ...
  const commands = [
    ...
    },
    {
      ...
      examples: [
        { cli: 'user -a create -n foo -p ~/.ssh/id_rsa.pub', desc: 'Creates an account named "foo", adding the pubkey located at ~/.ssh/id_rsa.pub to the list of accepted pubkeys.' },
        { cli: 'user -a add-pubkey -n foo -p ~/.ssh/id_rsa.secondary.pub', desc: 'Adds the key located at ~/.ssh/id_rsa.secondary.pub to the list of accepted pubkeys for the user account "foo".' },
        { cli: 'user -a remove-pubkey -n foo -p ~/.ssh/id_rsa.secondary.pub', desc: 'Removes the key located at ~/.ssh/id_rsa.secondary.pub from the list of accepted pubkeys for the user account "foo".' },
        { cli: 'user -d remove -n foo', desc: 'Removes the user account "foo" and all associated pubkeys from the server.' }
      ],
      options: {
        accountname,
        'action': {
    ...
    },
    {
      ...
      options: {
        ...
        'permission': {
          alias: 'p',
          demand: true,
          choices: [ PERMISSION_GRANT, PERMISSION_DENY ],
